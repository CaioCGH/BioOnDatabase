...
async function seedSpecies(parsedData) {
  ...
  const COL_END_OBSERVATIONS =
    "Sítio Rio Piavu (Camburi, Municipio de São Sebastião)";
  ...
  for (let i = 0; i < parsedData.length; i++) {
    ...
    while (key !== COL_START_OBSERVATIONS) {
      ...
      if (t(speciesRow[key]) == "S") {
        registerSpecialLocalities(speciesRow, key);
      }
      key = keys[++keyIndex];
    }
    registerSpecialLocalities(speciesRow, 'Tudo');
      ...
      if (speciesRow[key] != "") {
        ...
        const observation = new Observation({
          ...
          "Nome Científico": t(
            t(speciesRow["Gênero"]) +
              " " +
              t(speciesRow["Espécie"]) +
              " " +
              t(speciesRow["Subespécie"])
          ),
    ...
    process.stdout.write(
      Math.trunc((i / parsedData.length) * 1000) / 10 + "% completed\r"
    );
  }
}
...
async function findLocality(headerName) {
  ...
  locality = await Locality.findOne({
    "Nome Banco de Dados": headerName,
  }).exec();
  if (locality) {
    return locality;
  }
  locality = await Locality.findOne({
    "Nome Banco de Dados": t(headerName),
  }).exec();
  if (locality) {
    return locality;
  }
    ...
    if (!isNaN(arr[0][0])) {
      ...
      for (let i = 0; i < arrOfObserversAcronyms.length; i++) {
        const observer = await Observer.findOne({
          sigla: arrOfObserversAcronyms[i],
        });
        if (observer) {
...
async function registerSpecialLocalities(speciesRow, key) {
  const locality = await findLocality(key);
  const observation = new Observation({
    _id: generateId(),
    "Nome Científico": t(
